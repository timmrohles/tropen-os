...
<!-- GUIDE: Configure CI to require all tests pass before merge. Set branch protection rules in GitHub/GitLab. -->
...
<!-- GUIDE: Configure coverage tool (jest, nyc, etc.) with threshold enforcement in package.json or CI config. -->
...
### R4 — Testing Pyramid Balance [CRITICAL] [PREVENTED]

<!-- GUIDE: Monitor test execution time in CI. Flag PRs that add slow tests without justification. -->
...
**Enforced by:** Test Metrics (PREVENTED, coverage: partial)
...
| R4 | Test Metrics | CI | PREVENTED | partial |
