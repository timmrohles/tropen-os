...
import {
  WarningOctagon, Warning, Info, Note, CaretDown, CaretUp, CaretRight,
...
const AGENT_BADGE: Record<AgentSource, { label: string; bg: string; color: string }> = {
  security:      { label: 'Sec',   bg: 'color-mix(in srgb, var(--error) 15%, transparent)', color: 'var(--error-dark)' },
  architecture:  { label: 'Arch',  bg: 'color-mix(in srgb, var(--accent) 15%, transparent)',         color: 'var(--accent-dark)' },
  ...
  testing:       { label: 'Test',  bg: 'color-mix(in srgb, var(--accent) 10%, transparent)', color: 'var(--accent-dark)' },
  performance:   _neutral('Perf'),
  platform:      _neutral('CI/CD'),
  api:           _neutral('API'),
  'cost-awareness': _neutral('Cost'),
  scalability:   _neutral('Scale'),
  accessibility: { label: 'A11y', bg: 'color-mix(in srgb, var(--accent) 10%, transparent)', color: 'var(--accent-dark)' },
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          ...
          {(fixingIds.size > 0 || groupBatchFixingIds.size > 0) && (
            <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
              <PhosphorSpinner size={11} weight="bold" style={{ animation: 'spin 1s linear infinite', color: 'var(--accent)' }} aria-hidden="true" />
              <span style={{ fontSize: 11, color: 'var(--accent)' }}>
                {fixingIds.size + groupBatchFixingIds.size > 1
                  ? `${fixingIds.size + groupBatchFixingIds.size} Fixes…`
                  : 'Fix läuft…'}
              </span>
              <button
                className="btn btn-ghost btn-sm"
                onClick={handleAbortFixes}
                style={{ fontSize: 11, display: 'flex', alignItems: 'center', gap: 3, color: 'var(--error)' }}
              >
          ...
          <span style={{ fontSize: 11, color: 'var(--text-tertiary)' }}>
            {groupedItems.length} · {totalFindings} Findings
          </span>
        </div>
      </div>

      {/* Single filter row: severity tabs left, dropdowns right */}
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 14, flexWrap: 'wrap' }}>
        {/* Severity tabs */}
        <div style={{ display: 'flex', alignItems: 'center', gap: 0, flex: 1, flexWrap: 'wrap' }}>
          {severityChips.map(({ value, label }, i) => (
            <button
              key={value}
              onClick={() => setFilter('severity', value)}
              style={{
                background: 'none', border: 'none', cursor: 'pointer',
                padding: '6px 4px', marginLeft: i > 0 ? 6 : 0,
                minHeight: 32, fontSize: 13, fontWeight: severityFilter === value ? 600 : 400,
                color: severityFilter === value ? 'var(--text-primary)' : 'var(--text-tertiary)',
                borderBottom: severityFilter === value ? '1px solid var(--text-primary)' : '1px solid transparent',
                whiteSpace: 'nowrap',
              }}
            >
              {label}
            ...
            {groupedItems.flatMap((item) => {
              ...
              const groupMainRow = (
                <tr key={`group-${item.ruleId}`} style={{
                  ...
                  {/* Merged: chevron + dot + title + tags + actions */}
                  <td colSpan={6} style={{ padding: '9px 8px', verticalAlign: 'middle' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                      ...
                      <span style={{
                        ...
                        background: SEVERITY_COLOR[item.severity], flexShrink: 0,
                      }} aria-hidden="true" />
                      ...
                      <span style={{
                        ...
                        border: '1px solid var(--border)', color: 'var(--text-tertiary)', whiteSpace: 'nowrap',
                      }}>
                        {item.count} Dateien
                      </span>
                      <span style={{
                        ...
                        border: `1px solid ${SEVERITY_COLOR[item.severity]}`,
                      }}>
