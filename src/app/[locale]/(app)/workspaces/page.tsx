  ...
  return (
    <div className="content-max" aria-busy={loading}>
      <div className="page-header">
        <div className="page-header-text">
          <h1 className="page-header-title">
            <ShareNetwork size={22} color="var(--accent)" weight="fill" aria-hidden="true" />
            {t('title')}
          </h1>
        ...
        <div className="page-header-actions">
          <button className="btn btn-primary" onClick={() => { setCreating(true); setError('') }}>
            <Plus size={14} weight="bold" aria-hidden="true" /> {t('newWorkspace')}
          </button>
        </div>
      </div>
      ...
      {creating && (
        <div className="card" style={{ padding: 20, marginBottom: 20 }}>
          <form onSubmit={handleCreate} style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            ...
            <div>
              <label style={fieldLabel}>{t('descriptionLabel')}</label>
      ...
      <div style={{ display: 'flex', gap: 6, marginBottom: 20 }}>
        ...
        >
          {t('tabActive')} {workspaces.length > 0 && `(${workspaces.length})`}
        </button>
        <button
        ...
        >
          ...
          {t('tabArchived')} {archived.length > 0 && `(${archived.length})`}
        </button>
      </div>
