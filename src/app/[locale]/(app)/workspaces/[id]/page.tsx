...
export default function WorkspaceDetailPage() {
  ...
  const TABS: { key: Tab; label: string; icon: React.ReactNode }[] = [
    { key: 'items',    label: t('tabItems'),    icon: <Database    size={14} weight="bold" aria-hidden="true" /> },
    { key: 'members',  label: t('tabMembers'),  icon: <Users       size={14} weight="bold" aria-hidden="true" /> },
    { key: 'comments', label: t('tabComments'), icon: <SquaresFour size={14} weight="bold" aria-hidden="true" /> },
    { key: 'settings', label: t('tabSettings'), icon: <GearSix    size={14} weight="bold" aria-hidden="true" /> },
  ]
  ...
  return (
    <div className="content-max">
      ...
      >
        <ArrowLeft size={14} weight="bold" /> {t('backToAll')}
      </button>
      ...
      <div className="page-header">
        ...
        <div className="page-header-actions">
          <span style={{ fontSize: 12, color: 'var(--text-tertiary)' }}>
            {t('itemsAndMembers', { items: items.length, members: members.length })}
          </span>
        </div>
      </div>
        ...
        {TABS.map(tabItem => (
          <button
            ...
            style={{
              ...
              fontSize: 13, fontWeight: tab === tabItem.key ? 600 : 400,
              color: tab === tabItem.key ? 'var(--text-primary)' : 'var(--text-tertiary)',
