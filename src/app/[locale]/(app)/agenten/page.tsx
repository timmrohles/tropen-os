            ...
            {(selectedAgent || creatingAgent) && (
              <div className="card" style={s.editCard}>
                <div style={s.row}>
                  ...
                  <input style={inp} value={agentForm.name} onChange={e => setAgentForm(f => ({ ...f, name: e.target.value }))} placeholder={t('namePlaceholder')} />
                </div>
                <div style={s.row}>
                  <label style={s.label}>{t('descriptionLabel')}</label>
                  <input style={inp} value={agentForm.description} onChange={e => setAgentForm(f => ({ ...f, description: e.target.value }))} placeholder={t('descriptionPlaceholder')} />
                </div>
                <div style={s.row}>
                  <label style={s.label}>{t('systemPromptLabel')}</label>
                  <textarea style={{ ...textarea, minHeight: 120 }} value={agentForm.system_prompt} onChange={e => setAgentForm(f => ({ ...f, system_prompt: e.target.value }))} placeholder={t('systemPromptPlaceholder')} />
                </div>
                <div style={s.row}>
                ...
                <div style={s.actions}>
                  <button className="btn btn-primary" onClick={handleAgentSave} disabled={agentSaving || !agentForm.name.trim()}>
                    <FloppyDisk size={14} weight="bold" /> {agentSaving ? tc('saving') : tc('save')}
                  </button>
                  {selectedAgent && (
                  ...
                  {selectedAgent && (
                    agentDeleteConfirm ? (
                      <>
                        ...
                        <button className="btn btn-danger" style={{ marginLeft: 'auto' }} onClick={handleAgentDelete}>{t('deleteConfirmYes')}</button>
                        <button className="btn btn-ghost" onClick={() => setAgentDeleteConfirm(false)}>{tc('cancel')}</button>
                      </>
                    ) : (
