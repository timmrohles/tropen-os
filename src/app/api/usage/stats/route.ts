...
function computeKpis(rows: UsageRow[]) {
  ...
  return { totalCost, requestCount, activeUsers, topModel }
}

function computeCo2(rows: UsageRow[]): { co2Min: number; co2Max: number } {
...
function buildChartData(chartRaw: { cost_eur: number | null; created_at: string }[] | null) {
...
function buildWsTable(rows: UsageRow[]) {
  const wsMap: Record<string, { name: string; requests: number; cost: number; models: Record<string, number> }> = {}
  for (const r of rows) {
    if (!wsMap[r.workspace_id]) {
      wsMap[r.workspace_id] = { name: r.workspaces?.name ?? r.workspace_id, requests: 0, cost: 0, models: {} }
    }
  ...
  return Object.values(wsMap).sort((a, b) => b.cost - a.cost).map(ws => ({
    ...ws,
...
function buildUserTable(rows: UsageRow[]) {
  const userMap: Record<string, { name: string; requests: number; cost: number; lastActive: string }> = {}
  for (const r of rows) {
    const u = r.users
    if (!u) continue
    if (!userMap[r.user_id]) {
      userMap[r.user_id] = { name: u.full_name ?? u.email, requests: 0, cost: 0, lastActive: r.created_at }
    }
  ...
  return Object.values(userMap).sort((a, b) => b.cost - a.cost).map(u => ({
    ...u,
...
export const GET = withOrgAdmin(async (req, { auth }) => {
  ...
  try {
    ...
    const { totalCost, requestCount, activeUsers, topModel } = computeKpis(rows)
    const { co2Min, co2Max } = computeCo2(rows)
    const chartData = buildChartData(chartRaw as { cost_eur: number | null; created_at: string }[] | null)
    const wsTable = buildWsTable(rows)
    const userTable = buildUserTable(rows)
